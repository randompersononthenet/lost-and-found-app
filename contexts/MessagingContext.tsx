  ...
  const sendImage = useCallback(async (conversationId: string, localUri: string) => {
    ...
    try {
      ...
      const doUpload = async () => {
        // Convert to Uint8Array for better RN/Web compatibility
      ...
      try {
        ...
        console.warn('Upload attempt 1 failed:', e1);
        // quick retry once
      ...
      const msg = error?.message || (typeof error === 'string' ? error : 'Could not send image. Please try again.');
      Toast.show({ type: 'error', text1: 'Image Send Failed', text2: msg });
    }
  }, [user, loadConversations]);
